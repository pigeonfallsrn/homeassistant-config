...
var dispatchPrefix = "mxmp-dispatch-event-";
...
var SESSION_STORAGE_PLAYER_ID = "mxmp-player-id";
...
var Volume = class extends i$5 {
	...
	render() {
      ...
      <div class="volume" slim=${this.slim || E}>
        ...
        <mxmp-icon-button hide=${this.store.hidePower()} @click=${this.togglePower} .path=${mdiPower} style=${buttonStyle}></mxmp-icon-button>
      </div>
    `;
	}
	getMax() {
...
var MusicAssistantService = class {
	...
	*/
	async discoverMassQueueConfigEntryId() {
		const massQueue = (await this.hass.callWS({ type: "config_entries/get" })).find((entry) => entry.domain === "mass_queue" && entry.state === "loaded");
		if (!massQueue?.entry_id) throw new Error(MASS_QUEUE_NOT_INSTALLED);
...
var Store = class {
	...
	getActivePlayerFromStorage() {
		return window.sessionStorage.getItem("mxmp-player-id") || "";
	}
...
var CardEditor = class extends BaseEditor {
	...
	renderTabContent() {
		...
		const t = (s, sec) => x`<mxmp-section-tab .schema=${s} .section=${sec} .config=${c} .hass=${h}></mxmp-section-tab>`;
		return r(this.activeTab, [
			[Tab.COMMON, () => x`<mxmp-common-tab .config=${c} .hass=${h}></mxmp-common-tab>`],
			[Tab.PLAYER, () => x`<mxmp-player-tab .config=${c} .hass=${h}></mxmp-player-tab>`],
			[Tab.MEDIA_BROWSER, () => x`<mxmp-media-browser-tab .config=${c} .hass=${h}></mxmp-media-browser-tab>`],
	...
	render() {
      ...
      <mwc-list multi class="list">
        ${itemsWithFallbacks(this.items, this.config).map((item) => {
			return x`
            <mxmp-media-row @click=${() => this.dispatchEvent(customEvent(MEDIA_ITEM_SELECTED, item))} .item=${item} .store=${this.store}></mxmp-media-row>
          `;
		})}
      </mwc-list>
    `;
...
HaMediaPlayerBrowse = __decorate$1([t$3("mxmp-ha-media-player-browse")], HaMediaPlayerBrowse);
//#endregion
...
var START_PATH_KEY$1 = "mxmp-media-browser-start";
var MediaBrowserBrowser = class extends i$5 {
	...
	updateIsCurrentPathStart() {
		this.isCurrentPathStart = localStorage.getItem(START_PATH_KEY$1) === JSON.stringify(this.navigateIds);
	}
	render() {
      ...
      ${i(this.layout, x`<mxmp-ha-media-player-browse
...
__decorate$1([e$3("mxmp-ha-media-player-browse")], MediaBrowserBrowser.prototype, "mediaBrowser", void 0);
//#endregion
//#region src/sections/media-browser/media-browser-section.ts
var START_PATH_KEY = "mxmp-media-browser-start";
var LAYOUT_KEY = "mxmp-media-browser-layout";
...
__decorate$1([e$3("mxmp-media-browser-browser")], MediaBrowser.prototype, "browserComponent", void 0);
//#endregion
...
var QUEUE_SEARCH_STORAGE_KEY = "mxmp-queue-search-state";
...
var QueueSearch = class extends i$5 {
	...
	runSearch(continueFromCurrent) {
		...
		this.currentMatchIndex = getCurrentMatchIndex(matchIndices, continueFromCurrent, this.matchIndices[this.currentMatchIndex] ?? -1);
...
__decorate$1([e$3("mxmp-queue-search-panel")], QueueSearch.prototype, "panel", void 0);
...
var QueueList = class extends i$5 {
	...
	scrollToItem(index) {
		this.shadowRoot?.querySelectorAll("mxmp-media-row")?.[index]?.scrollIntoView({
			behavior: "smooth",
			block: "center"
		});
	}
...
var QueueController = class {
	...
	async scrollToCurrentlyPlaying() {
		...
		(this.host.shadowRoot?.querySelectorAll("mxmp-media-row")[this.selectedQueueIndex])?.scrollIntoView({
			behavior: "smooth",
			block: "center"
		});
	}
...
//#endregion
//#region src/sections/queue/queue-mxmp-utils.ts
...
var QueueSonos = class extends i$5 {
	...
	async refetchAndScroll() {
		...
		const pos = this.store.activePlayer.attributes.queue_position;
		if (pos) this.shadowRoot?.querySelector("mxmp-queue-list")?.scrollToItem(pos - 1);
	}
...
var Queue = class extends i$5 {
	render() {
		return this.store.config.entityPlatform === "music_assistant" ? x`<mxmp-queue-mass .store=${this.store}></mxmp-queue-mass>` : x`<mxmp-queue-sonos .store=${this.store}></mxmp-queue-sonos>`;
	}
	static {
		this.styles = i$8`
    :host {
...
var LOCAL_STORAGE_KEY = "mxmp-search-state";
      ...
      <div class="list" ?hidden=${this.browseLoading || this.browseResults.length === 0}>
        <mwc-list multi>
          ${this.browseResults.map((browseItem, index) => {
			return x` <mxmp-media-row @click=${() => this.onBrowseItemClick(index)} .item=${toMediaPlayerItem(browseItem)} .store=${this.store}></mxmp-media-row> `;
		})}
        </mwc-list>
      </div>
    `;
...
__decorate$1([e$3("mxmp-search-results")], Search.prototype, "searchResults", void 0);
__decorate$1([e$3("mxmp-search-bar")], Search.prototype, "searchBar", void 0);
//#endregion
...
var Volumes = class extends i$5 {
	...
	volumeWithName(player, updateMembers = true, groupName = "") {
      ...
      <div class="switches" ?hidden=${hideSwitches}>
        <mxmp-source .store=${this.store} .player=${player}> </mxmp-source>
