...
cmd_ui_automations() {
    ...
    # Get all automation entities from HA
    curl -s "http://supervisor/core/api/states" -H "Authorization: Bearer $SUPERVISOR_TOKEN" 2>/dev/null | \
    python3 << 'PYEOF'
...
# Get all automation IDs from YAML files
yaml_ids = set()
    ...
    for filepath in glob.glob(pattern):
        try:
            with open(filepath, 'r') as f:
                content = f.read()
                # Simple regex to find automation IDs
                import re
                ids = re.findall(r"id:\s*['\"]?([a-zA-Z0-9_-]+)['\"]?", content)
                yaml_ids.update(ids)
        except:
            pass

# Find UI-only automations
...
cmd_doctor() {
    ...
    if [ -n "$(cd /config && git status --short)" ]; then
        echo "  ⚠ Uncommitted changes detected - run 'hac push' to sync"
    fi
