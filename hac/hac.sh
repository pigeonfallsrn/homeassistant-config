...
cmd_health() {
    ...
    if [ "$count" -gt 0 ]; then
        echo "  ⚠ RETIRED S24 (sm_s928u): $count refs still in YAML"
        grep -rl "sm_s928u" /homeassistant/packages/ /homeassistant/automations.yaml 2>/dev/null | while read f; do echo "    → $(basename $f)"; done
    else
        echo "  ✓ No dead notify targets found"
    fi
    echo ""
...
try:
    ...
    u=[x for x in s if x['state'] in ('unavailable','unknown') and not x['entity_id'].startswith(('automation.','scene.','script.','group.'))]
    by_domain={}
    for e in u:
        d=e['entity_id'].split('.')[0]
        by_domain[d]=by_domain.get(d,0)+1
    print(f'  Total unavailable/unknown: {len(u)}')
    for d,c in sorted(by_domain.items(),key=lambda x:-x[1])[:8]:
        print(f'    {d}: {c}')
except Exception as e:
    print(f'  Error: {e}')
"
    echo "$(date +%Y-%m-%d)" > /homeassistant/hac/.last_health_check
    echo ""
    echo "✓ Health check complete — $(date '+%Y-%m-%d %H:%M')"
}
...
cmd_mcp() {
    echo "═══════════════════════════════════════════════════════════════════════════════"
    echo "HAC v$HAC_VERSION - CLAUDE MCP SESSION — $(date '+%Y-%m-%d %H:%M')"
    echo "═══════════════════════════════════════════════════════════════════════════════"
    echo ""
    echo "## HANDOFF"
    cat "$HAC_DIR/HANDOFF.md" 2>/dev/null || echo "No HANDOFF.md found"
    echo ""
    echo "## RECENT COMMITS"
    GIT_PAGER=cat git -C /homeassistant log --oneline -5 2>/dev/null || echo "Git unavailable"
    echo ""
    echo "## CONFIG CHECK"
    ha core check 2>/dev/null || echo "ha core check unavailable"
    echo ""
    echo "## CRITICAL RULES SUMMARY"
    head -30 "$HAC_DIR/CRITICAL_RULES.md" 2>/dev/null || echo "No CRITICAL_RULES.md found"
    echo ""
    echo "═══════════════════════════════════════════════════════════════════════════════"
    ...
    echo ""
    echo "## SYSTEM CONTEXT"
    echo "Van: 2012 Honda Odyssey | HFL BT MAC 00:0A:30:9A:20:F5 | BT audio only, no Android Auto"
    echo "Network: UniFi UDM Pro SE | Main 192.168.1.x | IoT VLAN 192.168.21.x"
    echo "Notify ACTIVE: notify.mobile_app_john_s_s26_ultra"
    echo "Notify DEAD - NEVER USE: notify.mobile_app_sm_s928u"
    echo "SBM switches: NEVER in light.turn_off actions (see CRITICAL_RULES)"
    echo "Privacy: no GPS/MAC/IP in exports or gists"
    echo ""
    echo "## MCP CANNOT SEE - DO NOT ASSUME"
    echo "X YAML file contents - use terminal grep, not MCP"
    echo "X Presence accuracy - person.john_spencer lags 5-10min"
    echo "X ESPHome flash state / firmware / OTA logs"
    echo "X UniFi topology / VLAN assignments / AP locations"
    echo "! ha_deep_search may return stale cached index - terminal grep is authoritative"
    echo "! last_triggered != automation is correct or useful"
    echo ""
    echo "## ROUTING - Which AI for What"
    echo "YAML edits / entity debug / fix  -> this session (MCP + terminal)"
    echo "Bulk audit 170 automations       -> hac gem (Gemini 2.5 Pro, 1M context)"
    echo "Spreadsheet / trend analysis     -> hac gpt (ChatGPT Code Interpreter)"
    echo "Research / best practice         -> hac project (Claude.ai browser)"
    echo ""
    if [ -f "/homeassistant/hac/.last_health_check" ]; then
        last=$(cat /homeassistant/hac/.last_health_check)
        now_ts=$(date +%s)
        last_ts=$(date -d "$last" +%s 2>/dev/null || echo $now_ts)
        days=$(( (now_ts - last_ts) / 86400 ))
        if [ "$days" -gt 14 ]; then
            echo "HEALTH CHECK OVERDUE: last run $last (${days}d ago) - run: hac health"
        else
            echo "Last health check: $last (${days}d ago)"
        fi
    else
        echo "HEALTH CHECK NEVER RUN - run: hac health"
    fi
}
cmd_route() {
    cat << 'ROUTE'
═══════════════════════════════════════════════════════════════
HAC ROUTING - Which AI for What?
═══════════════════════════════════════════════════════════════
YAML edits / automation fix / entity debug
  -> hac mcp (Claude Desktop + MCP, LOCAL DEV connector)
  -> Direct file access + live HA states

Bulk audit 100+ automations / large context analysis
  -> hac gem (Gemini 2.5 Pro, 1M token context)
  -> Upload: hac export then paste LLM JSON

Spreadsheet / export / trend analysis
  -> hac gpt (ChatGPT Code Interpreter)
  -> Upload FULL Excel from hac export

Research / best practice / planning
  -> hac project (Claude.ai browser)
  -> Web search + HA community access

Quick question / no context needed
  -> Claude.ai browser (no context paste needed)

TOKEN EFFICIENCY RULES
  hac pkg <file>      Load one package only
  hac health --quick  Red flags only, fast
  hac mcp             Full context for complex work
  Never paste full config to Claude.ai browser
═══════════════════════════════════════════════════════════════
ROUTE
}

cmd_update() {
    echo "=== HA Release Check ==="
    local current=$(ha core info 2>/dev/null | grep -i version | head -1)
    echo "Current: $current"
    echo ""
    echo "Fetching latest release info..."
    local latest=$(curl -s --max-time 10 "https://version.home-assistant.io/stable.json" 2>/dev/null | python3 -c "import json,sys; d=json.load(sys.stdin); print('Latest: ' + d.get('homeassistant','unknown'))" 2>/dev/null || echo "Latest: check https://www.home-assistant.io/blog/")
    echo "$latest"
    echo ""
    echo "Domains to watch for your system:"
    echo "  esphome | notify | template | zha | matter | cover | ratgdo"
    echo ""
    echo "Release notes: https://www.home-assistant.io/blog/categories/release-notes/"
    echo "Breaking changes: https://www.home-assistant.io/docs/installation/updates/"
}
...
cmd_sheets() {
    ...
    /usr/bin/python3 /homeassistant/python_scripts/export_to_sheets.py
    if [ $? -eq 0 ]; then
        echo ""
...
-------------------------------------------------------------------------------
  ...
  hac route     | Which AI for which task (MCP/Gemini/GPT/Claude.ai)
...
cmd_deadend() {
    ...
    case "$subcmd" in
        list)
            ...
            echo "═══ Dead Ends [$status] ═══"
            python3 /homeassistant/hac/hac_knowledge.py list_dead_ends "$status"
            ;;
        add)
            [ -z "$1" ] && echo "Usage: hac deadend add <id> <summary> <method> <result> [hypothesis]" && return
            local de_id="$1" summary="$2" method="$3" result="$4" hypothesis="${5:-}"
            local out=$(python3 /homeassistant/hac/hac_knowledge.py add_dead_end "$de_id" "$summary" "$method" "$result" "$hypothesis")
            log_ok "Dead end: $out"
            ;;
        resolve)
            [ -z "$1" ] && echo "Usage: hac deadend resolve <id> <resolution>" && return
            local de_id="$1" resolution="$2"
            local out=$(python3 /homeassistant/hac/hac_knowledge.py resolve_dead_end "$de_id" "$resolution")
            if [ "$out" = "RESOLVED" ]; then
                log_ok "Resolved: $de_id"
            else
                log_warn "Not found: $de_id — check: hac deadend list all"
            fi
            ;;
        all)
            echo "═══ All Dead Ends ═══"
            python3 /homeassistant/hac/hac_knowledge.py list_dead_ends all
            ;;
        *)
            echo "Usage: hac deadend [list|add|resolve|all]"
            ;;
    esac
}

cmd_slice() {
    local keyword="${1:-general}"
    echo "═══════════════════════════════════════════════════════════════"
    echo "  HAC Context Slice: $keyword"
    echo "═══════════════════════════════════════════════════════════════"
    python3 /homeassistant/hac/hac_knowledge.py slice "$keyword"
    echo ""
...
cmd_wrap() {
    ...
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  Paste this into Claude to finish wrap:"
    echo ""
    local active=$(grep "^TASK:" "$HAC_DIR/ACTIVE.md" 2>/dev/null | head -1 | sed "s/TASK: //")
    local commits=$(git -C /homeassistant log --oneline -2 2>/dev/null | tr "
" " | ")
    echo "  HAC WRAP $(date +%Y-%m-%d) | $active | commits: $commits"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
}
cmd_close() {
    echo "═══════════════════════════════════════════════════════════════"
    ...
    # 1. Knowledge stats
    echo "── Knowledge status ──"
    python3 /homeassistant/hac/hac_knowledge.py stats
    echo ""

    # 2. Promote candidates check
    local candidates=$(python3 /homeassistant/hac/hac_knowledge.py promote_candidates)
    if echo "$candidates" | grep -q "\["; then
        echo "── Promote candidates (times_hit >= 3, not yet in CRITICAL_RULES) ──"
        echo "$candidates"
        echo ""
        printf "  Promote any to CRITICAL_RULES now? [y/N]: "
        read -r promote_ans
        if [ "$promote_ans" = "y" ] || [ "$promote_ans" = "Y" ]; then
            echo "  Run: hac promote \"<text>\" for each item above"
        fi
        echo ""
    fi
    ...
    # 3. Git status check
    echo "── Git status ──"
    local git_status=$(git -C /homeassistant status --short 2>/dev/null)
    ...
    # 4. Backup retention cull (keep 5 most recent per base filename)
    echo "── Backup retention ──"
    local culled=0
    for base in hac.sh CRITICAL_RULES.md kitchen-wall-v2; do
        local count=$(ls "$BACKUP_DIR"/${base}.* 2>/dev/null | wc -l)
        if [ "$count" -gt 5 ]; then
            local excess=$((count - 5))
            ls "$BACKUP_DIR"/${base}.* 2>/dev/null | sort | head -"$excess" | while read f; do
                rm -f "$f"
                echo "  pruned: $(basename $f)"
            done
            culled=$((culled + excess))
        fi
    done
    if [ "$culled" -eq 0 ]; then
        log_ok "Backups within retention limit"
    else
        log_ok "Pruned $culled old backup(s)"
    fi
    echo ""

    # 5. Active task reminder
    echo "── Active task ──"
    if [ -f "$HAC_DIR/ACTIVE.md" ]; then
        head -4 "$HAC_DIR/ACTIVE.md"
    fi
    log_ok "To update active task: hac active \"task\""
    echo ""

    # 6. Export reminder
    echo "── Export ──"
    log_ok "Run hac export manually if needed"
    echo ""

    echo "═══════════════════════════════════════════════════════════════"
    # Write HANDOFF.md for next session orientation
    local handoff="$HAC_DIR/HANDOFF.md"
    local today=$(date '+%Y-%m-%d %H:%M')
    local commits=$(git -C /homeassistant log --oneline -3 2>/dev/null | sed 's/^/  /')
    local active=$(grep -A2 "^TASK:" "$HAC_DIR/ACTIVE.md" 2>/dev/null | head -3 | sed 's/^/  /')
    local backlog=$(grep -m3 "^- \[" "$HAC_DIR/BACKLOG_kitchen_dashboard.md" 2>/dev/null | head -3 | sed 's/^/  /')
    cat > "$handoff" << HANDOFF
# HAC Handoff — $today
...
HANDOFF
    log_ok "HANDOFF.md written — cat hac/HANDOFF.md to orient next session"

    echo "  Session closed $(date '+%Y-%m-%d %H:%M')"
    echo "═══════════════════════════════════════════════════════════════"
}
