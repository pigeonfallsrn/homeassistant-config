...
cmd_close() {
    ...
    if [ -z "$git_status" ]; then
        ...
        if [ -n "$changed_files" ]; then
            auto_msg="hac: update ${changed_files% }"
        fi

        printf "  Commit message [enter to use auto / type to override]:\n"
        printf "  > $auto_msg\n"
        printf "  > "
        read -r commit_msg
    ...
    printf "\n  Update ACTIVE.md for next session? [y/N]: "
    read -r active_ans
    if [ "$active_ans" = "y" ] || [ "$active_ans" = "Y" ]; then
        echo "  Run: hac active \"next task description\""
    fi
    echo ""

    # 6. Export reminder
    echo "── Export ──"
    printf "  Run hac export to sync to Drive? [y/N]: "
    read -r export_ans
    if [ "$export_ans" = "y" ] || [ "$export_ans" = "Y" ]; then
        if command -v hac >/dev/null 2>&1; then
            hac export
        else
            log_warn "hac export not available in this context — run manually"
        fi
    fi
    echo ""
