...
**1. HA Terminology (2026 Update)**
- Home Assistant rebranded "Add-ons" → "Apps" in 2026
- CLI still uses `ha addons` for backwards compatibility
- CLI warning: "The use of 'addons' is deprecated, please use 'apps' instead!"
- **Correct terminology:** Apps (not add-ons)

**2. Dual Ecosystems Require Hybrid Solutions**
- User has 40% Matter-capable Echo devices, 60% legacy
- Best approach: Matter Hub (RiDDiX) + Alexa Devices integration
- Don't force single solution when hybrid is better

**3. "If You Have to Ask, You Don't Use It"**
- Community best practice for app removal decisions
- Apps running unused waste resources
- Easy to reinstall if needed later

**4. Synology Integration > HA Add-on**
- External hardware (NAS, UniFi controller) = use integration
- Don't duplicate services with HA apps when external exists
- Integration points to external service, app runs duplicate instance

**5. Terminal Users vs Visual Editors**
- Two distinct user profiles in HA community
- Terminal users: SSH + vim/nano + File Editor = sufficient
- Visual editors: Studio Code Server = essential
- Know your workflow, optimize for it

### HAC Workflow Enhancements

**Best Practices Confirmed:**
- Document decisions in real-time during work
- One commit per logical change
- Use meaningful commit messages
- Track "Tabled Projects" for future work
- Record learnings for knowledge retention

**Git Commit Pattern:**
```bash
cd /homeassistant && \
git add hac/session_YYYYMMDD.md && \
git commit -m "docs: [concise summary]" && \
git push
```

**Session Structure:**
1. Critical Fixes (immediate problems)
2. Integration Work (new functionality)
3. Optimization (cleanup/efficiency)
4. Learnings (knowledge capture)
5. Tabled Projects (future work)

### Research Methodology

**When to Research:**
- User asks "is this useful/needed?"
- Conflicting information in memory
- New technology/recent changes (Matter, etc.)
- Community best practices needed

**Research Sources Priority:**
1. Official HA documentation
2. HA Community forums (recent posts)
3. GitHub project pages (check maintenance status)
4. Technical blogs (with dates)
5. Cross-reference multiple sources

**Red Flags in Research:**
- Project archived/discontinued (t0bst4r Matter Hub)
- Alpha/beta software for production use
- Community consensus against approach
- Abandoned >6 months with no fork
...
- Official/maintained by HA team
- Resource usage justified by benefit
- Future-proof (Matter Server)

**Remove Apps When:**
- Running but unconfigured (Assist Microphone)
- Redundant with better solution (Nginx + Cloudflared)
- Abandoned by developer (Matter Hub alpha)
- Not part of workflow (Studio Code for terminal users)
- One-time installer completed (Get HACS)

**Hybrid Approach When:**
- Split ecosystem (Matter + legacy Echo devices)
- Multiple use cases (Music Assistant for automation + Spotify app for casual)
- Complementary not duplicate (local + cloud)

---

## Tabled Projects - Detailed Planning

### Project 1: Alexa/Google Home Integration
**Status:** Research complete, ready for implementation

**Components:**
- RiDDiX Matter Hub (actively maintained fork)
- Alexa Devices integration (TTS, announcements)
- Google Home integration (via Matter or cloud)

**Timeline:** When ready to implement voice control
**Dependencies:** None, can start anytime
**Estimated time:** 2-3 hours

---

### Project 2: Comprehensive Media Ecosystem (NEW)
**Status:** Planning required - see session_20260214.md

**User's Media Sources:**
- HDHomeRun antenna → Local TV channels
- Plex Pass (lifetime)
- Spotify Premium
- Amazon Music (Prime)
- Netflix
- Personal MP3 collection (location TBD)
- SiriusXM (lifetime subscription)

**Local Radio Stations Requested:**
- 102.3 WHTL
...
**Sports/Specialty:**
- Hudl Fan (Whitehall Memorial School)

**Questions to Research:**
1. Music Assistant: Best provider for each source?
2. HDHomeRun + Plex integration for local TV
3. Local FM radio streaming options in HA
4. MP3 storage: Synology NAS vs HA media folder
5. SiriusXM integration possibilities
6. Hudl Fan streaming availability

**Timeline:** Next dedicated session
**Dependencies:** Music Assistant already running
...
**Duration:** ~3 hours
**Focus:** Automation architecture investigation + workflow optimization

### Accomplishments

1. ✅ **Deep Research: UI vs YAML Automations**
   - Investigated "176 UI automations" warning
   - Discovered actual architecture: 151 YAML automations
   - Confirmed package-based organization is best practice

2. ✅ **Tools Created**
   - `count_automations.py` - Accurate automation counter
   - Scans all sources: packages/, automations/, automations.yaml, configuration.yaml
   - Compares with entity registry, identifies orphaned entities

3. ✅ **Documentation Created**
   - `automation_detection_workflow.md` - Troubleshooting reference
   - `automation_architecture.md` - System overview
   - `research_findings_ui_vs_yaml.md` - Complete investigation
   - `learnings/20260213.md` - Session learnings

4. ✅ **Workflow Perfected**
   - Quick command available for future sessions
   - Search patterns documented
   - HAC detector failure root cause identified

### Key Learnings

**Architecture Validation:**
- 151 YAML automations (121 packages, 25 automations/, 4 UI, 1 inline)
- Package-based organization = industry best practice
- Already version controlled in Git
- No migration needed

**Technical Insights:**
- Entity IDs generated from `alias:` field, not `id:` field
- HAC detector was scanning wrong locations
- Entity registry != automation storage

**Research Quality:**
- Official HA documentation
- Community forums (100+ automation patterns)
- HA blog posts (2020-2026)
- Git version control best practices

### Files Modified
- Created: hac/scripts/count_automations.py
- Created: hac/notes/automation_detection_workflow.md
- Created: hac/notes/automation_architecture.md
- Created: hac/notes/research_findings_ui_vs_yaml.md
- Updated: hac/learnings/20260213.md
- Updated: hac/session_20260213.md

### Next Session Prep
- HAC detector fix (scan packages/ properly)
- Consider adding `hac auto-analysis` command
- Potential cleanup of 49 orphaned entities

**Session Status:** Complete ✅
**Git Status:** All changes committed and pushed
**Knowledge Base:** Updated with permanent reference materials

- 15:15: Claude Project 'HA HAC System/Workflow' configured with SYSTEM_KNOWLEDGE.md + custom instructions - replaces gist-based context for Claude sessions
